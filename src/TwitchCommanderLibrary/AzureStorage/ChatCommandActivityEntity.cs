...
{
	public class ChatCommandActivityEntity : ITableEntity
	{
		...
		public void Save(AzureStorageSettings azureStorageSettings, TableNames tableNames)
		{
			...
			if (string.IsNullOrWhiteSpace(tableNames.ChatCommandActivity)) throw new SettingMissingException(nameof(tableNames.ChatCommandActivity));
			if (string.IsNullOrWhiteSpace(PartitionKey))
				...
				AzureStorageHelper.GetTableClient(azureStorageSettings, tableNames.ChatCommandActivity).UpsertEntity(this);
		}
				...
				ChatCommandResult chatCommandResult = ChatCommandResult.Executed)
		{
			...
			chatCommandActivityEntity.Save(azureStorageSettings, tableNames);
		}
		...
		public static void Save(
			...
			TableNames tableNames,
				...
				ChatCommandResult chatCommandResult)
		{
			Save(azureStorageSettings, tableNames, channel, chatter, command, request, response, DateTimeExtensions.UnixTimeInSecondsToDateTime(requestedDateTime), chatCommandResult);
		}
		...
		public static void Save(
			...
			string response)
		{
			Save(azureStorageSettings, tableNames, channel, chatter, command, request, response, DateTime.UtcNow, ChatCommandResult.Executed);
		}
			...
			string chatterName = "")
		{
			...
			if (!string.IsNullOrWhiteSpace(chatterName))
				response = AzureStorageHelper.GetTableClient(azureStorageSettings, tableNames.ChatCommandActivity)
		...
		public static double SecondsSinceLastRequest(AzureStorageSettings azureStorageSettings, TableNames tableNames, string channelName, string commandName, string chatterName = "")
		{
			ChatCommandActivity queryResponse = GetLastCommandRequest(azureStorageSettings, tableNames, channelName, commandName, chatterName);
