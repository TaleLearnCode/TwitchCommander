...
namespace TaleLearnCode.TwitchCommander.AzureStorage
{
	...
	public class BotTimerEntity : IBotTimer, ITableEntity
	{
		...
		public void Save(AzureStorageSettings azureStorageSettings)
		{
			...
			if (string.IsNullOrWhiteSpace(azureStorageSettings.TimerBotTableName)) throw new SettingMissingException(nameof(azureStorageSettings.TimerBotTableName));
			...
			AzureStorageHelper.GetTableClient(azureStorageSettings, azureStorageSettings.TimerBotTableName).UpsertEntity(this);
		}
			...
			DateTime? lastExecution = null)
		{
			...
			botTimerEntity.Save(azureStorageSettings);
		}

		public static void BotTimerExecuted(BotTimer botTimer, AzureStorageSettings azureStorageSettings, DateTime? executionTime = null)
		{
			botTimer.LastExecution = (executionTime != null) ? (DateTime)executionTime : DateTime.UtcNow;
			FromBotTimer(botTimer).Save(azureStorageSettings);
		}

		public static List<BotTimer> Retrieve(AzureStorageSettings azureStorageSettings, string channelName)
		{
			return ToBotTimerList(AzureStorageHelper.GetTableClient(azureStorageSettings, azureStorageSettings.TimerBotTableName).Query<BotTimerEntity>(t => t.PartitionKey == channelName.ToLower()).ToList());
		}
